...
baharhist <- function(baslik, xlabel, ylabel, xlimiti, ylimiti, renk){
  ...
  limitlermatrixi[1,1]<-65.62
  limitlermatrixi[2,1]<-70.0
  limitlermatrixi[1,2]<-70.01
  limitlermatrixi[2,2]<-75.0
  limitlermatrixi[1,3]<-75.01
  limitlermatrixi[2,3]<-79.58
  plot(NULL, xlim=c(65,80), ylim=c(0,60), main = "Var4-Histogram", xlab = xlabel, ylab=ylabel)
  for(j in 1:100){
...
baharhist(baslik = "HÝSTOGRAM", ylabel =" y ekseni", xlabel="x ekseni", xlimiti = c(65,80), ylimiti=c(0,60), renk="Purple")
