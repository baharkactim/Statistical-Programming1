...
baharhist <- function(baslik, xlabel, ylabel, xlimiti, ylimiti, renk){
  ...
  limitlermatrixi[1,1]<-85.05
  limitlermatrixi[2,1]<-88.0
  limitlermatrixi[1,2]<-88.01
  limitlermatrixi[2,2]<-90.0
  limitlermatrixi[1,3]<-90.01
  limitlermatrixi[2,3]<-92.17
  
  plot(NULL, xlim=c(85,93), ylim=c(0,100), main = "Var7-Histogram", xlab = xlabel, ylab=ylabel)
  for(j in 1:100){
...
baharhist(baslik = "HÝSTOGRAM", ylabel =" y ekseni", xlabel="x ekseni", xlimiti = c(85,93), ylimiti=c(0,100), renk="Purple")
